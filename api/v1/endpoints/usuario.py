...
# GET Usuário
@router.get('/{usuario_id}', response_model=UsuarioSchemaVagas, status_code=status.HTTP_202_ACCEPTED)
async def get_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
        usuario: UsuarioSchemaVagas = result.scalars().unique().one_or_none()
...
async def delete_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
        usuario_del: UsuarioSchemaVagas = result.scalars().unique().one_or_none()
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_session)):
    usuario = await autentitar(email=form_data.username, senha=form_data.password, db=db)
